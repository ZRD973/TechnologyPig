            ...
            <t-table border="2" border-color="#ffffff"  @change="change" class="table_table">
                <view class="table_head">
                    <t-tr font-size="14"  style="line-height: 30px;background:#E7F3F1 ;" color="#95b99e" align="left">
                        ...
                        <t-th align="center">所属单位</t-th>
                        <t-th align="center">操作</t-th>
                    </t-tr>
                </view>
                <view class="table_body">
                    <t-tr font-size="12" color="#5d6f61" align="right" v-for="(item, index) in from.tabledata" :key="item.id">
                        <t-td align="center" >{{ index+1 }}</t-td>
                        <t-td align="center" >{{ item.Name }}</t-td>
                        <t-td align="center" >{{ item.DeptCap }}</t-td>
...
<script>
    ...
    export default {
        ...
        computed: {
            reversedMessagedept(){
                return this.from.arraydept[this.from.indexdept].value
            },
        },
        methods:{
            ...mapGetters(['deptList']),
            ...
            async searchdata(){
                let reldept = this.from.arraydept;
                let deptdata = '';
                if(reldept=[]){
                    deptdata = '';
                }else{
                    deptdata = this.from.arraydept[this.from.indexdept].lable;
                }
                const [err, data] = await helper.req({
                    uri: '/PageQuery',
                    data: {
                        path: 'hongbo/PersonList.txt',
